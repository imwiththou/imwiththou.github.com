...
jQuery( document ).ready( function() {
	var selected = 0;
	if( jQuery.cookie( 'cleanretina_tab' ) ) {
		selected = jQuery.cookie( 'cleanretina_tab' );
		jQuery.cookie( 'cleanretina_tab', null );
	}
	
	var tabs = jQuery( '#cleanretina_tabs' ).tabs( { selected: selected } );
	
	jQuery( '#wpbody-content form' ).submit( function() {
		var selected = tabs.tabs( 'option', 'selected' );
		jQuery.cookie( 'cleanretina_tab', selected );
	} );
